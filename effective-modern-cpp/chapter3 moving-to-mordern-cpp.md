...
-    还有一个是模板别名：
            ...
            template<typename T> 
            using myalloc=std::list<T, MyAlloc<T>>;
...
default underlying type.\
    ...
    **默认类型是int,可以当做别的类型来使用**
        
        enum class Status;  // 使用的默认的int
        enum class Status: std::uint32_t;   //使用uint32_t
...
forward-declared only if their declaration specifies an underlying type.
    **enum class 可以声明而不被定义，so-called forward-declaration**
    也就是在将enum class的声明和实现分离，实现低耦合
    
        enum class Status;
        void continuePro(Status S);    //当Status的实现发生改变的时候，不需要修改
...
-   对于vector，如果capacity不够用，在重新申请的时候，c\++98赋予push_back强的异常安全性，保证在copy-c的时候发生异常会
    回到原来的状态，但是move则不行，不能把已经move过去的再move回来,否则会抛异常，所以对于push_back只有当被操作对象保证move OP是noexcept 的时候才会使用
-   对于swap函数只有在low-level的成员是noexcept的时候，upper-level swap 才是noexcept
...
-   在98标准库的实现中，多数push_back都有强的异常安全保证，然后可以保证如果出现异常，原来的对象不会出现问题\
    ...
    只有在move不会抛出异常的时候，这样也就是说，只有当相应的最底层的实现是nonexcept的时候，container才会使用move版本的
    push_back.(STL一般情况下, 是有两种分别使用copy和move的push_back)
            ...
            //这实现看起来有点蛋疼啊
-   **Note**,noexcept是接口的一部分，意味被外部依赖，需要谨慎使用。如果今天用，明天就改了，这样可能使用client原来的代码需要修改，that wouldn't be what you want.
-   c++11中所有的内存分配函数默认都是noexcept，无论是否进行声明。因为内存分配时候出现exception会很难受\
    除了这些类中含有的data member 含有一些对象可能会抛except的时候，即显式的noexcept(false);
-   functions with **wide contracts** 意思是他没有先决条件，对于传入的参数没有限制，这样的函数不会显示未定义行为, 那样你可以大胆的声明它为noexcept,对于narrow ones，如果判断条件是由caller保证的，那该函数调用也不会出现exception，也可以声明noexcept,
...
-   书上说**c++11中constexpr 成员函数implicitly是const的，在c\++14中取消了这样的限制**(但我测试的gcc加--std=c++14 仍然不可用(8.1可以), vs2017可以的)
...
-   声明 copy operation(constructor or assignment) 会默认停止 move operation (constructor or assignment)的生成，原因是**当你自定义一个copy operation 时候,**
    **就已经认定对于这个class 默认的 copy不适合这个类的使用，那么move也可能不合适，反过来也是这样，如果定义了copy operation相应的move operation也不会默认的生成**
